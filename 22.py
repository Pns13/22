...
\t##`````##````##`````##````##`````##````##`````##
\t##`````##````##`````##````#########````##`````##
			{m}•{k}•{h}•{sir} KENDAK KABA LA {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
#		method.append('touch')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[purple]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} HARAP BERSABAR {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{u}>>{k} Good Bye Dadaahh{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Gunawkah teserah kaba \n\t{x}——> {h}amaw adaw  Bug/Error Ngiciak awu\n\t{x}——> {h}Doa ka ajaw aku sehat \n\t{x}——> {h}Dapat bini alap\n\t{x}——> {h}Aminkah mangku kamu dapat amal{x}')
	time.sleep(3)
	login()
